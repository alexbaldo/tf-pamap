...
# 1 input channel
# 1024 features
W_conv1 = weight_variable([ 64, 8, 1, 1024 ])
b_conv1 = bias_variable([ 1024 ])
h_conv1 = tf.nn.relu( conv2d( x_repr, W_conv1 ) + b_conv1 )
h_pool1 = max_pool( 64, 10, h_conv1 )

# L2 - CONVOLUTIONAL LAYER
# 4x2 patch
# 1 input channel
# 4096 features
W_conv2 = weight_variable([ 4, 2, 1024, 4096 ])
b_conv2 = bias_variable([ 4096 ])
...
# 4096 features
# W * 40 neurons
W_fc1 = weight_variable([ 4 * 4 * 4096, W * 40 ])
b_fc1 = bias_variable([ W * 40 ])
h_pool2_flat = tf.reshape( h_pool2, [-1, 4 * 4 * 4096] )
